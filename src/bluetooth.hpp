...
#define BLE_SEARCH_DURATION_SECONDS 1
    ...
    void bluetoothCode(void *pvParameters)
    {
        ...
        for (;;)
        {
            ...
            auto res = BLEDevice::getScan()->start(BLE_SEARCH_DURATION_SECONDS, false);
            Serial.println("Bluetooth scanning finished.");

            // call callback with all found devices
            std::vector<BLEAdvertisedDevice> v(res.getCount());
            for (auto i = 0, n = res.getCount(); i < n; i++)
            {
